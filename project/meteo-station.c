...
extern resource_t res_meteo, res_watch_triggered, res_event;
extern char* res_serial_data;
int watch_triggered = 0;
int watch_activated = 0;

/*
 * Meteo station: print the meteorological predictions
 * and send to the watch the weather if it is activated.
 */

PROCESS(meteo_station_receiver, "Meteo station receiver");
PROCESS(meteo_station_transmitter, "Meteo station transmitter");
AUTOSTART_PROCESSES(&meteo_station_receiver, &meteo_station_transmitter);

uip_ipaddr_t watch_ipaddr;

#define NUMBER_OF_URLS 1
char *service_urls[NUMBER_OF_URLS] = { "meteo" };
...
/*---------------------------------------------------------------------------*/
PROCESS_THREAD(meteo_station_transmitter, ev, data)
{
  PROCESS_BEGIN();
  ...
  while(1) {
    ...
    if (watch_activated && watch_triggered) {
      printf("--Toggle watch--\n");
...
/*---------------------------------------------------------------------------*/
PROCESS_THREAD(meteo_station_receiver, ev, data)
{
  PROCESS_BEGIN();
